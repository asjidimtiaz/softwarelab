...
import { Users, Target, Zap, TrendingUp, ArrowUpRight, ArrowDownRight, Download, ArrowRight, CheckSquare } from "lucide-react";
...
export default async function AnalyticsPage() {
  ...
  const stats = [
    { label: "Total Leads", value: statsData.totalLeads.toLocaleString(), change: "+12.5%", trending: "up", icon: Users },
    { label: "Hot Leads", value: statsData.hotLeads.toString(), change: "+5.2%", trending: "up", icon: Target },
    { label: "Avg. Response", value: "1.2h", change: "-18%", trending: "down", icon: Zap },
  ...
  return (
    <div className="space-y-10 pb-12">
      ...
      <div className="flex flex-col gap-6">
        <div>
           <p className="text-xs font-semibold text-muted-foreground/60 mb-2 uppercase tracking-wider">Mon, July 7</p>
           <h1 className="text-3xl font-bold tracking-tight text-foreground">Hello, Courtney</h1>
                ...
                <CardContent className="p-10 pt-0">
                    <div className="space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-[10px] font-bold text-muted-foreground">• 2 tasks</span>
                        </div>
                        
                        <div className="space-y-4">
                            {[
                                { name: "One-on-One Meeting", priority: "High", due: "Today", color: "rose" },
                                { name: "Send a summary email to stakeholders", priority: "Low", due: "3 days left", color: "blue" }
                            ].map((task, i) => (
                                <div key={i} className="flex items-center justify-between p-6 rounded-[1.5rem] bg-secondary/30 hover:bg-secondary/50 transition-colors group cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        <div className={cn("w-3 h-3 rounded-full", task.color === "rose" ? "bg-rose-500" : "bg-blue-500")} />
            ...
            <Card className="rounded-3xl border-border bg-white shadow-sm overflow-hidden">
                <CardHeader className="p-8 pb-4">
                    <div className="flex items-center gap-4">
                        ...
                        <CardTitle className="text-lg font-bold">My Goals</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="p-10 pt-4 pb-12">
                    <div className="space-y-10">
                        {[
                            { label: "Check Emails and Messages", percentage: 73, color: "bg-primary" },
                            { label: "Prepare a brief status update to the client", percentage: 11, color: "bg-orange-400" },
                            { label: "Update project documentation", percentage: 63, color: "bg-emerald-400" }
                        ].map((goal, i) => (
        ...
        <div className="lg:col-span-5 space-y-6">
            <Card className="rounded-3xl border-border bg-white shadow-sm p-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                    {[
                        { name: "Product launch", tasks: 6, team: 12, color: "bg-primary/10 text-primary" },
                        { name: "Team brainstorm", tasks: 2, team: 32, color: "bg-blue-100 text-blue-600" },
                        { name: "Branding launch", tasks: 4, team: 9, color: "bg-emerald-100 text-emerald-600" }
                    ].map((project, i) => (
