            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <AnimatedSection direction="right">
                ...
                <h1 className="text-6xl md:text-8xl font-black tracking-tighter mb-8 leading-[0.9]">
                  <span className="text-white block">{tech.title.split(' ').slice(0, -1).join(' ')}</span>
                  <span className="text-gradient from-primary to-brand-gold block">{tech.title.split(' ').slice(-1)}</span>
                </h1>
                <p className="text-xl md:text-2xl text-zinc-400 font-medium mb-12 max-w-xl leading-relaxed">
                  {tech.description}
                </p>
                <Link
                  href={`/${locale}/quote`}
